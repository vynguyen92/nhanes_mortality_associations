#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
################  FUNCTION TO CREATE A PANEL OF DENSITY PLOTS OF PARTICIPANTS BY MEASUREMENTS  ################
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#

# Purpose: This function creates a panel of density plots to show the number of participants across the 
#          distribution of the physiological indicators. 
#
# Inputs: dataset_long - long-formatted dataset based on the physiological indicators
...
#
# Outputs: none - png and pdf versions of the panel of density plots are printed to a folder


density_pi_distribution <- function(dataset_long
                                      , current_directory
                                      , pi_names_body_syst_cats)
{
  ...
  # Define the name of the new folder to contain the densitys
  name_folder <- "Density Plots - Distribution of Physiological Indicators"
  ...
  # Initialize an empty list to store the density plots for all the physiological indicators
  list_density <- list()
  
  # Define a string vector of alphabetical letters to label each density
  ...
  # Make a density for each physiological and store it in a list
  for(i in seq(num_pi))
  {
    ...
    # Define a data frame corresponding to the body system color for this physiological indicator
    ...
    # Make the density plot of the number of participants by each unique measurement of the physiological indicator
    density_i <- ggplot() +
      geom_density(data = subset(dataset_long
                                   , pi == pi_i)
                     , aes(x = pi_value)
                     , fill = color_i)  +
      xlab(pi_name_i) +
    ...
    # Store the density plot and the title label into a list
    list_density[[i]] <-  arrangeGrob(density_i
                                         , top = title_i)
  }
  
  # Make a panel of density plots of all physiological indicators with 3 columns and 9 rows
  panel_density <- do.call("grid.arrange"
                             , c(list_density
                                 , ncol = 3))
  
  # Add a legend of body system colors at the top of the panel of density plots
  # Add an y-axis label 
  panel_density <- grid.arrange(legend_body_system
                                  , arrangeGrob(panel_density
  ...
  # Define file names of the png and pdf versions of the panel of density plots 
  plot_name.png <- paste("density_panel"
                         , "0_100"
                         , "pi_mortality.png"
                         , sep = "_")
  plot_name.pdf <- paste("density_panel"
                         , "0_100"
                         , "pi_mortality.pdf"
                         , sep = "_")
  
  
  # Save the panel of density plots as a png and pdf
  print(plot_name.png)
